...
import {
  ArrowRight, Brain, CalendarBlank, CaretLeft, ChatCircleText, CheckCircle,
...
export default function App() {
  ...
  return <main className="family-app">
    ...
    {screen === 'report' ? <ReportScreen onBack={() => setScreen('route')} onSubmit={report} loading={loading} error={error} /> : <section id="family-content" className="family-workspace" tabIndex={-1}>
      ...
      </motion.div><FamilyNav current={screen} onChange={navigate} t={t} />
    </section>}
  </main>;
}
...
function FamilyNav({ current, onChange, t }: { current: Exclude<FamilyScreen, 'report'>; onChange: (screen: Exclude<FamilyScreen, 'report'>) => void; t: Translate }) { const items = [{ id: 'route', labelKey: 'navHome', Icon: House }, { id: 'agenda', labelKey: 'navAgenda', Icon: CalendarBlank }, { id: 'notebook', labelKey: 'navNotebook', Icon: NotePencil }, { id: 'documents', labelKey: 'navDocuments', Icon: FileText }, { id: 'team', labelKey: 'navTeam', Icon: UsersThree }, { id: 'assistant', labelKey: 'navHelp', Icon: ChatCircleText }] as const; return <nav className="family-nav" aria-label="Secciones principales">{items.map(({ id, labelKey, Icon }) => <button key={id} className={current === id ? 'active' : ''} onClick={() => onChange(id)} aria-current={current === id ? 'page' : undefined}><Icon weight={current === id ? 'fill' : 'regular'} /><span>{t(labelKey)}</span></button>)}</nav>; }
