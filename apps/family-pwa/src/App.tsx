...
function FamilyLogin({ onSession }: { onSession: (session: Session) => void }) {
  ...
  const submitRegistration = async (event: React.FormEvent) => { event.preventDefault(); setLoading(true); setError(''); setNotice(''); try { const result = await familyApi.register({ ...registration, dni: onlyDigits(registration.dni), consent_confirmed: true }); setNotice(result.message); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No pudimos registrar tu solicitud.'); } finally { setLoading(false); } };
  return <main className="family-login healthcare-login"><section><div className="family-login-brand"><Heartbeat weight="fill" /> <strong>Ruta Viva</strong></div><p>{mode === 'login' ? 'ACCESO FAMILIAR' : 'SOLICITUD DE REGISTRO'}</p><h1>{mode === 'login' ? 'Acompaña su ruta paso a paso.' : 'Empecemos con tus datos.'}</h1><span>{mode === 'login' ? 'Consulta el seguimiento, las próximas coordinaciones y avisa si aparece una dificultad.' : 'El equipo revisará los datos antes de habilitar el acceso.'}</span><div className="access-tabs" role="tablist" aria-label="Acceso familiar"><button type="button" role="tab" aria-selected={mode === 'login'} onClick={() => { setMode('login'); setError(''); setNotice(''); }}>Ingresar</button><button type="button" role="tab" aria-selected={mode === 'register'} onClick={() => { setMode('register'); setError(''); setNotice(''); }}>Registrarme</button></div>{mode === 'login' ? <form onSubmit={submitLogin}><label>DNI<input inputMode="numeric" maxLength={16} autoComplete="username" value={dni} onChange={(event) => setDni(onlyDigits(event.target.value))} /></label><label>Contraseña<input type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>{error && <div className="family-error" role="alert">{error}</div>}<button disabled={loading}><Key weight="fill" />{loading ? 'Verificando…' : 'Ingresar a mi ruta'}</button></form> : <form onSubmit={submitRegistration} className="registration-form"><label>DNI del acompañante<input required inputMode="numeric" maxLength={16} value={registration.dni} onChange={(event) => updateRegistration('dni', onlyDigits(event.target.value))} /><small>Solo números. Se verifica antes de habilitar el acceso.</small></label><label>Tu nombre completo<input required autoComplete="name" value={registration.companion_name} onChange={(event) => updateRegistration('companion_name', event.target.value)} /></label><label>Nombre del niño, niña o adolescente<input required value={registration.patient_name} onChange={(event) => updateRegistration('patient_name', event.target.value)} /></label><label>Vínculo contigo<input required placeholder="Madre, padre, tutor/a…" value={registration.relationship} onChange={(event) => updateRegistration('relationship', event.target.value)} /></label><label>Teléfono de contacto<input required inputMode="tel" autoComplete="tel" value={registration.phone} onChange={(event) => updateRegistration('phone', event.target.value)} /></label><label>Distrito<input required value={registration.district} onChange={(event) => updateRegistration('district', event.target.value)} /></label><label className="consent-check"><input required type="checkbox" checked={registration.consent_confirmed} onChange={(event) => updateRegistration('consent_confirmed', event.target.checked)} /><span>Confirmo que estos datos se usarán para revisar mi solicitud y coordinar una ruta.</span></label>{error && <div className="family-error" role="alert">{error}</div>}{notice && <div className="family-notice" role="status">{notice}</div>}<button disabled={loading || Boolean(notice)}><ShieldCheck weight="fill" />{loading ? 'Enviando…' : notice ? 'Solicitud enviada' : 'Enviar solicitud'}</button></form>}{DEMO_CREDENTIALS && mode === 'login' && <small>Demostración con datos sintéticos precargados.</small>}</section></main>;
}
